...
# ----------------------------------------------------------------------
def load_prices(
    tickers: list[str], begin: str, end: str, interval: str
) -> pd.DataFrame:
...
def regression_stats_alternative(
    asset_returns: pd.Series, market_returns: pd.Series
) -> dict:
...
def risk_decomposition(reg: dict) -> dict:
    ...
    return {
        ...
        'systematic_fraction': variance_systematic
        / (variance_systematic + variance_idiosyncratic),
    }
...
def main(
    assets: list[str] = typer.Option(
        default=[ASSET], help='Asset ticker(s) (e.g. QCI.DE NVD.DE)'
    ),
    assets_fractions: list[float] = typer.Option(
        default=[],
        help='Portfolio weight per asset (e.g. 0.6 0.4); must sum to 1 and match number of assets. Omit for equal weights.',
    ),
    market: str = typer.Option(
        default=MARKET, help='Market index ticker (e.g. ^GDAXI)'
    ),
    ...
    var_confidence: float = typer.Option(
        default=VAR_CONFIDENCE, help='VaR confidence level (e.g. 0.99 for 99%)'
    ),
    portfolio_value: float = typer.Option(
        default=PORTFOLIO_VALUE, help='Portfolio value in currency units'
    ),
):
    if assets_fractions:
        if len(assets_fractions) != len(assets):
            raise SystemExit(
                f'assets_fractions length ({len(assets_fractions)}) must match assets length ({len(assets)}).'
            )
        if abs(sum(assets_fractions) - 1.0) > 1e-6:
            raise SystemExit(
                f'assets_fractions must sum to 1 (got {sum(assets_fractions):.6f}).'
            )
    ...
    prices = load_prices(
        [*assets, market], f'{begin}-01-01', f'{end}-12-31', INTERVAL
    )
    ...
    for asset in assets:
        ...
        print(
            f'=== (On any day there is {100 - int(var_confidence * 100)}% probability that position of {portfolio_value:,.0f} in {asset} loses more than...) ==='
        )
        print(
            f'  Historical: {hvar:7.4%}  -> {hvar * portfolio_value:,.0f} EUR '
            f'on {portfolio_value:,.0f}  (based on what actually happened)'
        )
        print(
            f'  Parametric: {pvar:7.4%}  -> {pvar * portfolio_value:,.0f} EUR '
            f'on {portfolio_value:,.0f}  (based on a normal distribution fitted to the data)'
        )
        print(
            f'  Gap: a much larger historical VaR indicates fat tails  '
        ...
        print(
            f"  Beta (slope)        : {reg['beta']:.3f}  (measures an asset's sensitivity to market moves)"
        )
        print(
            f'  Alpha (annualised)  : {reg["alpha_per_period"] * PERIODS_PER_YEAR:.4%}  (return above what CAPM predicts; positive = outperformed)'
        )
        print(
            f'  R^2                 : {reg["r_squared"]:.3f}  (fraction of asset variance explained by the market)'
        )
        print(
            f'  p-value (beta)      : {reg["p_value"]:.2e}'
        ...
        print(
            f'  CAPM E[R] (annual)  : {capm:.4%}  (CAPM - Capital Asset Pricing Model - expected return)'
        )
    ...
    if len(assets) > 1:
        ...
        print(f'\n{"=" * 70}')
        print(
            f'Portfolio ({", ".join(f"{asset} {weight:.0%}" for asset, weight in zip(assets, weights))})'
        )
        print(f'{"=" * 70}')
        print(
            f'  Covariance matrix (annualised):\n{cov_matrix.to_string()}'
...
def _make_prices(
    n: int = 500, beta: float = 1.2, noise: float = 0.01
) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {'asset': asset_prices, 'market': market_prices}, index=dates
    )
...
def test_parametric_var(_sample: dict[str, pd.Series]) -> None:
    ...
    qs_var = -qs.stats.value_at_risk(
        _sample['simple_asset'], confidence=VAR_CONFIDENCE
    )
    assert abs(ours - qs_var) < _ATOL
...
def test_beta(_sample: dict[str, pd.Series]) -> None:
    ...
    qs_beta = qs.stats.greeks(
        _sample['simple_asset'], _sample['simple_market']
    )['beta']
...
def test_annualised_volatility(_sample: dict[str, pd.Series]) -> None:
    ...
    qs_vol = qs.stats.volatility(
        _sample['simple_asset'], periods=PERIODS_PER_YEAR
    )
...
def test_regression_stats_alternative(_sample: dict[str, pd.Series]) -> None:
    ...
    reg = regression_stats(_sample['log_asset'], _sample['log_market'])
    alt = regression_stats_alternative(
        _sample['log_asset'], _sample['log_market']
    )
